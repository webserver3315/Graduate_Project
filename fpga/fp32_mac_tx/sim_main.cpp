...
void test_case(Vtop* top, VerilatedVcdC* wave_fp, int clk_i, int kazu){
    top->clk_i   = clk_i;
    top->eval();
    wave_fp->dump(kazu);

    float32 alpha, bravo, acc, delta, delta_sim, charlie;
    alpha.total = -0.5;
    bravo.total = 0.75;
    acc.total = 0;
    delta.total_int = top->delta_output;
...
int main(int argc, char** argv, char** env) {
    ...
    // aa.total = 0; bb.total = 0; acc.total_int = 0x00;
    aa.total = -0.5; bb.total = 0.75; acc.total_int = 0x00;
    float32 middle_delta;
    middle_delta.total = aa.total * bb.total;
    delta.total = aa.total * bb.total + acc.total;
    printf("%f * %f = %f\n", aa.total, bb.total, middle_delta.total);
    printf("%08x * %08x = %08x\n", aa.total_int, bb.total_int, middle_delta.total_int);
    printf("%f * %f + %f = %f\n", aa.total, bb.total, acc.total, delta.total);
    printf("%08x * %08x + %08x = %08x\n", aa.total_int, bb.total_int, acc.total_int, delta.total_int);    

    int clk_i = 0;
    srand(time(NULL));
    for(int i=0;i<100 ;i++){
        test_case(top, wave_fp, !clk_i, kazu);
        kazu++;
        clk_i = !clk_i;
    }
